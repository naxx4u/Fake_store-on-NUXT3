<template>
    ...
    <div v-if="cartStore.products.length > 0" class="flex w-full px-20 py-6 my-10 justify-center items-center   ">
        <div class="flex  gap-4  w-1/2 flex-col mx-auto">
            <div v-for="cartItem in cartStore.products" :key="cartItem.id"
            ...
            <button class="btn w-1/4">buy</button>
        </div>
        </div>
...
const totalPrice = computed(() =>
    cartStore.products.reduce((acc, product) => acc += product.price, 0)
)
